
-- ContextTypes
Insert into TAMW_CONTEXTTYPE (ID,V,NAME) values ('1','0','GLOBAL');
Insert into TAMW_CONTEXTTYPE (ID,V,NAME) values ('2','9','ENV');
Insert into TAMW_CONTEXTTYPE (ID,V,NAME) values ('3','3','DOMAIN');

-- Context
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('1','6','Global','1',null);
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('2','16','Dev','3','1');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('3','0','Int','3','1');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('4','0','Pro','3','1');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('5','0','B','2','2');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('6','0','C','2','2');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('7','0','V','2','2');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('8','0','W','2','2');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('9','0','X','2','2');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('10','0','Y','2','2');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('11','0','N','2','2');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('13','0','Z','2','2');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('14','0','U','2','2');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('15','0','S','2','2');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('16','0','Team','2','2');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('17','0','I','2','3');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('18','0','T','2','3');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('19','0','K','2','3');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('20','0','P','2','4');
Insert into TAMW_CONTEXT (ID,V,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('55601','1','Local','2','2');

Insert into TAMW_REVINFO (ID,TIMESTAMP,USERNAME,V) values ('1','1353165882016','test','0');

Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('1','1','1','Global','1',null);
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('2','1','1','Dev','3','1');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('3','1','1','Int','3','1');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('4','1','1','Pro','3','1');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('5','1','1','B','2','2');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('6','1','1','C','2','2');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('7','1','1','V','2','2');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('8','1','1','W','2','2');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('9','1','1','X','2','2');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('10','1','1','Y','2','2');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('11','1','1','N','2','2');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('13','1','1','Z','2','2');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('14','1','1','U','2','2');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('15','1','1','S','2','2');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('16','1','1','Team','2','2');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('17','1','1','I','2','3');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('18','1','1','T','2','3');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('19','1','1','K','2','3');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('20','1','1','P','2','4');
Insert into TAMW_CONTEXT_AUD (ID,REV,REVTYPE,NAME,CONTEXTTYPE_ID,PARENT_ID) values ('55601','1','1','Local','2','2');


--Permissions

Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1295','0','APP_TAB',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1297','0','EDIT_APP',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1298','0','EDIT_APP_SERVER',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1299','0','EDIT_RES_TYPE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1300','0','EDIT_RES',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1301','0','RES_TYPE_LIST_TAB',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1302','0','ENV_PANEL_LIST',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1303','0','PROP_LIST',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1304','0','RES_RESTYPE_TEMPLATE_LIST',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1305','0','EDIT_RESTYPE_TEMPLATE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1306','0','REL_RESTYPE_PANEL_LIST',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1307','0','SETTING_PANEL_LIST',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1308','0','CONSUMED_RES_LIST',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1309','0','PROVIDED_RES_LIST',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1310','0','BACK_TO_RES_LIST',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1311','0','APP_AND_APPSERVER_LIST',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1313','0','APP_LIST_ADDED_APPSERVER',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1314','0','RESOURCE_LIST',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1315','0','INSTANCE_TEMP_LIST',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1316','0','PROP_TYPE_NAME_VALUE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1317','0','NEW_RES',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1318','0','DELETE_RES',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1319','0','NEW_RESTYPE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1320','0','DELETE_RESTYPE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1321','0','EDIT_RES_OR_RESTYPE_NAME',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1322','0','ADD_NEW_ENV_OR_DOM',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1323','0','REMOVE_ENV_OR_DOM',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1324','0','EDIT_ENV_OR_DOM_NAME',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1325','0','EDIT_PROP_TYPE_NAME',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1326','0','EDIT_PROP_TYPE_VALIDATION',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1327','0','ADD_PROPTYPE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1328','0','DELETE_PROPTYPE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1329','0','SAVE_SETTINGS_PROPTYPE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1330','0','SAVE_SETTINGS_ENV',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1331','0','ADD_AS_PROVIDED_RESOURCE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1332','0','ADD_AS_CONSUMED_RESOURCE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1333','0','ADD_RELATED_RESOURCETYPE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1334','0','SAVE_PROPERTY',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1335','0','SAVE_RES_TEMPLATE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1336','0','RESET_PROP',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1337','0','EDIT_PROP_LIST_OF_INST_APP',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1338','0','DELETE_RES_TEMPLATE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1339','0','CREATE_TEMPLATE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1341','0','TAG_CURRENT_STATE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1342','0','CREATE_NEW_VERSION',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1343','0','SAVE_ALL_CHANGES',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1344','0','MANAGE_PROP_POPUP',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1345','0','ADD_REL_RESTYPE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1346','0','ADD_APPSERVER',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1347','0','DELETE_APPSERVER',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1348','0','ADD_APP',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1349','0','DELETE_APP',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1354','0','ADD_NEW_RES_OF_DEFAULT_RESTYPE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1355','0','ADD_APP_TO_APP_SERVER',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1356','0','DELETE_APP_TO_APP_SERVER',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1357','0','ADD_NODE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1359','0','EXCLUDE_APP_FROM_NODE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1360','0','ADD_APP_AND_APPSERVER',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1361','0','ADD_RELATED_RESOURCE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1362','0','DELETE_CONS_OR_PROVIDED_RELATION',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1363','0','DELETE_ROLE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1364','0','CREATE_ROLE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1365','0','ASSIGN_REMOVE_PERMISSION',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1366','0','SELECT_JBOSS_RUNTIME',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1367','0','EXPORT_CSV_DEPLOYMENTS',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1368','0','SAVE_NAME_AND_OR_TARGET_PLATFORM',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1369','0','EDIT_PROPERTIES_COMMENT_POPUP',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1370','0','ADD_NODE_RELATION',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1371','0','DELETE_NODE_RELATION',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1372','0','DECRYPT_PROPERTIES',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1373','0','EDIT_ALL_PROPERTIES',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1374','0','EDIT_NOT_DEFAULT_RES_OF_RESTYPE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1375','0','DELETE_RESTYPE_TEMPLATE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1376','0','EDIT_RES_TEMP',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1377','0','SAVE_RESTYPE_TEMPLATE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1378','0','SAVE_ALL_PROPERTIES',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1379','0','ROLES_AND_PERMISSIONS_TAB',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1380','0','SHAKEDOWN_TEST_PAGE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1381','0','ADD_SHAKEDOWN_TEST',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1382','0','SHAKEDOWN_TEST_MODE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1383','0','TEST_NEIGHBOURHOOD_CHECKBOX',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1384','0','EXECUTE_SHAKE_TEST_CHECKBOX',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1385','0','EXECUTE_SHAKE_TEST_ORDER',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1386','0','STP_MANAGEMENT_PAGE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1387','0','EDIT_STP',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1388','0','DELETE_STP',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1389','0','ADD_STP',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1390','0','RENAME_INSTANCE_DEFAULT_RESOURCE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1391','0','DELETE_RES_INSTANCE_OF_DEFAULT_RESTYPE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1392','0','SHOW_VALUE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1393','0','ADD_EVERY_RELATED_RESOURCE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1394','0','DELETE_EVERY_RELATED_RESOURCE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1395','0','V','7');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1396','0','X','9');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1397','0','Y','10');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1398','0','B','5');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1399','0','C','6');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1400','0','Team','16');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1401','0','U','14');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1402','0','W','8');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1403','0','I','17');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1404','0','K','19');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1405','0','N','11');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1406','0','T','18');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1407','0','Z','13');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1408','0','P','20');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1409','1','Local','55601');
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1559','0','MODIFY_APPSERVER_RELATION',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1561','0','CREATE_RELEASE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1562','0','EDIT_RELEASE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1563','0','DELETE_RELEASE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1564','0','VIEW_RELEASE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1566','0','COPY_FROM_RESOURCE',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1568','0','TEST_GENERATION',null);
Insert into TAMW_PERMISSION (ID,V,VALUE,CONTEXT_ID) values ('1569','0','CHANGE_RESOURCE_RELEASE',null);


-- Roles

Insert into TAMW_ROLE (ID,DELETABLE,DEPLOYABLE,NAME,V) values ('1052','0','0','viewer','0');
Insert into TAMW_ROLE (ID,DELETABLE,DEPLOYABLE,NAME,V) values ('1053','0','0','config_admin','1');
Insert into TAMW_ROLE (ID,DELETABLE,DEPLOYABLE,NAME,V) values ('1054','0','0','app_developer','0');
Insert into TAMW_ROLE (ID,DELETABLE,DEPLOYABLE,NAME,V) values ('1055','0','0','server_admin','1');
Insert into TAMW_ROLE (ID,DELETABLE,DEPLOYABLE,NAME,V) values ('1056','0','0','shakedown_tester','0');
Insert into TAMW_ROLE (ID,DELETABLE,DEPLOYABLE,NAME,V) values ('1057','0','0','shakedown_admin','0');
Insert into TAMW_ROLE (ID,DELETABLE,DEPLOYABLE,NAME,V) values ('1058','0','0','Permissions without assigned Role','2');
Insert into TAMW_ROLE (ID,DELETABLE,DEPLOYABLE,NAME,V) values ('1059','1','1','test_deployer','2');
Insert into TAMW_ROLE (ID,DELETABLE,DEPLOYABLE,NAME,V) values ('1060','1','1','developer_deployer','3');
Insert into TAMW_ROLE (ID,DELETABLE,DEPLOYABLE,NAME,V) values ('1061','1','1','env_management_deployer','0');
Insert into TAMW_ROLE (ID,DELETABLE,DEPLOYABLE,NAME,V) values ('1062','1','1','admin_deployer','0');

-- Role_Permission

Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1295');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1296');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1297');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1298');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1299');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1300');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1301');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1302');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1303');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1304');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1305');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1306');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1307');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1308');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1309');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1310');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1311');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1312');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1313');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1314');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1315');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1316');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1344');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1367');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1376');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1317');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1318');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1319');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1320');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1321');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1322');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1323');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1324');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1325');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1326');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1327');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1328');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1329');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1330');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1333');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1343');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1345');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1363');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1364');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1365');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1372');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1373');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1374');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1375');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1377');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1379');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1393');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1394');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1559');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1331');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1332');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1334');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1335');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1336');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1337');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1338');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1339');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1340');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1341');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1342');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1361');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1362');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1369');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1378');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1392');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1346');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1347');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1348');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1349');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1350');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1351');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1352');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1353');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1354');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1355');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1356');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1357');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1358');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1359');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1360');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1366');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1368');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1370');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1371');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1390');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1391');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1056','1380');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1056','1381');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1056','1383');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1056','1384');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1056','1385');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1057','1382');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1057','1386');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1057','1387');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1057','1388');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1057','1389');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1059','1395');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1059','1396');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1059','1398');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1060','1397');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1060','1399');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1060','1400');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1060','1401');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1060','1402');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1060','1405');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1060','1409');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1061','1403');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1061','1404');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1061','1406');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1061','1407');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1062','1408');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1561');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1562');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1563');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1052','1564');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1053','1566');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1054','1568');
Insert into TAMW_ROLE_PERMISSION (ROLES_ID,PERMISSIONS_ID) values ('1055','1569');

-- Resource Type

alter table TAMW_resourceType 
        drop constraint FK_5i0ygpeyat7xqfe6fv3w5a0ei;
        
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1','APPLICATION','0',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('2','APPLICATIONSERVER','2',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('3','NODE','0',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('4','Database','0',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('5','DB2','0','4');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('6','Oracle','0','4');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('7','Webservice','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1000','ActiveDirectory','0',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1656','HostSubSection','0','1650');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1550','MSSql','4','4');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1052','JBossLoginModule','0',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1053','Certificate','0',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1100','JBoss7Management','0',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1150','JspLoadBalancer','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1151','IMS','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1660','JmxRemote','0','1650');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1201','Elan','7',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1350','Mail','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1250','ModCluster','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1251','ZUser','4',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1252','Webgui','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1253','POS','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1254','SAP','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1255','SAP_Servicebus','1','1254');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1256','SAP_Mandanten','13','1254');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1257','HornetQ','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1258','Keystore','1','1053');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1259','Truststore','1','1053');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1400','CertLoginModule','1','1052');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1261','MQ','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1301','MONAS','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1700','Log4J','0',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1752','IZAPublic','0','1750');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1750','IZA','0',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1605','Queue','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1606','HornetQQueue','0','1605');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1607','MQQueue','1','1605');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1608','MSSql_Sqljdbc4','1','4');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1650','TomcatSpecific','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1659','CertificatesApr_tomcat','1','1650');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1652','ConnectorHttp','1','1650');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1653','ConnectorHttps','2','1650');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1654','ConnectorAjp','1','1650');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1655','ConnectorApr','1','1650');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1753','IZASecure','0','1750');
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1800','JGroups','1',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1850','User','0',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1900','ContentServer','0',null);
Insert into TAMW_RESOURCETYPE (ID,NAME,V,PARENTRESOURCETYPE_ID) values ('1950','Unimail','0',null);


alter table TAMW_resourceType 
        add constraint FK_5i0ygpeyat7xqfe6fv3w5a0ei 
        foreign key (parentResourceType_id) 
        references TAMW_resourceType;
        
        
--Release
Insert into TAMW_RELEASE (ID,NAME,DESCRIPTION,INSTALLATIONINPRODUCTION,V) values ('1','Past','Das Release fÃ¼r alle Ressourcen, welche erfasst wurden, bevor das Releasing-Feature eingefÃ¼hrt wurde','2000-01-01','0');
Insert into TAMW_RELEASE (ID,NAME,DESCRIPTION,INSTALLATIONINPRODUCTION,V) values ('3','RL-13.10',null,'2013-10-19','0');
Insert into TAMW_RELEASE (ID,NAME,DESCRIPTION,INSTALLATIONINPRODUCTION,V) values ('4','BW-14.04',null,'2014-01-14','0');
Insert into TAMW_RELEASE (ID,NAME,DESCRIPTION,INSTALLATIONINPRODUCTION,V) values ('5','RL-14.04',null,'2014-04-26','0');
Insert into TAMW_RELEASE (ID,NAME,DESCRIPTION,INSTALLATIONINPRODUCTION,V) values ('6','BW-14.24',null,'2014-04-12','0');
Insert into TAMW_RELEASE (ID,NAME,DESCRIPTION,INSTALLATIONINPRODUCTION,V) values ('7','BW-14.32',null,'2014-08-07','0');
Insert into TAMW_RELEASE (ID,NAME,DESCRIPTION,INSTALLATIONINPRODUCTION,V) values ('8','RL-14.10',null,'2014-10-18','0');
Insert into TAMW_RELEASE (ID,NAME,DESCRIPTION,INSTALLATIONINPRODUCTION,V) values ('9','BW-14.50',null,'2014-12-11','0');
Insert into TAMW_RELEASE (ID,NAME,DESCRIPTION,INSTALLATIONINPRODUCTION,V) values ('51','RL-13.04','xxxx','2013-08-13','0');
Insert into TAMW_RELEASE (ID,NAME,DESCRIPTION,INSTALLATIONINPRODUCTION,V) values ('100','RL-14.11',null,'2014-11-13','0');


--TAMW_PROPERTYTYPE

Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('1','Name','0','[A-Z|a-z]');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('2','eMail','0','.*@{1}.*');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('3','Number','0','[0-9]*');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('4','AnyNoSpaces','0','[^ ]*');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('5','Boolean','0','^true$|^TRUE$|^false$|^FALSE$');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('1000','Path','0','.*');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('1001','FileName','0','.*');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('1050','LogLevel','0','TRACE|DEBUG|INFO|WARN|ERROR');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('1100','Hostname','4','^([a-zA-Z0-9\$\{\}]|[a-zA-Z0-9\$\{\}][a-zA-Z0-9\-\$\{\}]{0,61}[a-zA-Z0-9])(\.([a-zA-Z0-9\$\{\}]|[a-zA-Z0-9\$\{\}][a-zA-Z0-9\-\$\{\}]{0,61}[a-zA-Z0-9\$\{\}]))*$');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('1101','JavaHeapSize','0','[0-9]+[mg]');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('1150','SVNPath','9','^amw_files/.+\|[^ |]+$|^config_std/.+\|[^ |]+$|^ssl/ssl_.+\|[^ |]+$|^rpms/config/.+\|[^ |]+$');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('1200','Password','0','[^ ]*');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('1201','KeyStoreType','1','^PKCS12|JKS|JCEKS$');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('1202','Port','0','^([0-9]{1,4}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-2][0-9]|6553[0-5])$');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('1203','Username','0','[^ ]*');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('1250','JndiName','0','^[^\n\r\t]+$');
Insert into TAMW_PROPERTYTYPE (ID,PROPERTYTYPENAME,V,VALIDATIONREGEX) values ('1300','JeeAuthMethod','1','BASIC|FORM|CLIENT-CERT');


-- sequences
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('permissionId','1569');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('contextId','210650');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('resourceRelationId','24350');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('deploymentId','22900');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('favoriteResourceId','5850');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('revinfoId','83940');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('propertyDescriptorId','5100');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('propertyId','19250');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('propertytypeId','1400');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('resourceId','225100');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('resourceRelationTypeId','2250');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('resourceTagId','1200');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('resourceTypeId','2100');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('shakedownStpId','1105');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('shakedownTestId','1450');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('targetPlatformId','1000');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('templateDescriptorId','5050');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('userSettingsId','3100');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('versionId','1300');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('roleId','1063');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('appServerRelationId','201');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('releaseId','200');
Insert into SAMW_SEQUENCES (SEQ_NAME,NEXT_VAL) values ('resourceGroupId','225000');
