...
public class CopyResourceResult {
	...
	// for testing
	private Set<CopyFailure> exceptionEnums = new HashSet<CopyFailure>();

	@Getter
	private Set<CopyInfo> changedResourceParameters = new HashSet<CopyResourceResult.CopyInfo>();
	@Getter
	private Map<Integer, CopyResult> changedTemplates = new HashMap<Integer, CopyResult>();
	@Getter
	private Map<Integer, String> skippedConsumedRelations = new HashMap<Integer, String>();
	@Getter
	private Map<Integer, String> skippedProvidedRelations = new HashMap<Integer, String>();
	@Getter
	@Setter
	...
	public class CopyResult {
		...
		private String name;
		@Getter
		private Set<CopyInfo> copyInfos = new HashSet<CopyResourceResult.CopyInfo>();
	...
	public enum CopyTarget {
		...
		private CopyTarget(String typeName) {
	...
	public List<String> getCopyResultInfosAsHTMLStrings() {
		...
		if (!changedResourceParameters.isEmpty()) {
			List<String> changedRes = new ArrayList<String>();
		...
		if (!changedTemplates.isEmpty()) {
			List<String> changedTempl = new ArrayList<String>();
		...
		if (!skippedConsumedRelations.isEmpty()) {
			infos.add(getResultInfosAsHtml("Skipped consumed relations", new ArrayList<String>(skippedConsumedRelations.values())));
		}

		// skipped provided relations
		if (!skippedProvidedRelations.isEmpty()) {
			infos.add(getResultInfosAsHtml("Skipped provided relations", new ArrayList<String>(skippedProvidedRelations.values())));
		}
		return infos;
	}
