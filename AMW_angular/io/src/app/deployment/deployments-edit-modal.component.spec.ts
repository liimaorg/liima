...
describe('DeploymentsEditModalComponent (with query params)', () => {
  ...
  it('should log unknown edit actions on doEdit', inject(
    [DeploymentsEditModalComponent],
    (deploymentsEditModalComponent: DeploymentsEditModalComponent) => {
      // given
      deploymentsEditModalComponent.editActions = [
        'Change date',
        'Confirm',
        'Reject',
        'Cancel'
      ...
      deploymentsEditModalComponent.deployments = [
        { id: 1, selected: true } as Deployment
      ];
      spyOn(console, 'error');
  ...
  it('should apply date for confirmation', inject(
    [DeploymentsEditModalComponent],
    (deploymentsEditModalComponent: DeploymentsEditModalComponent) => {
      // given
      ...
      const deployment1: Deployment =
        deploymentsEditModalComponent.deployments[0];
      const deployment2: Deployment =
        deploymentsEditModalComponent.deployments[1];
  ...
  it('should clear data after doEdit()', inject(
    [DeploymentsEditModalComponent],
    (deploymentsEditModalComponent: DeploymentsEditModalComponent) => {
      // given
      const newDeploymentDate: string = '30.11.2017 09:19';
      ...
      // then
      expect(deploymentsEditModalComponent.confirmationAttributes).toEqual(
        {} as Deployment
      );
