...
Access web application on: http://localhost:8080/AMW_angular/
...
From *Run/Debug configurations* in IntelliJ select ```Debug application remotely on docker``` and click on debug (
Shift+F9) in order to debug application from IDE.
...
* H2 JDBC url: jdbc:h2:<path to liima folder>
  /AMW_business/src/test/resources/integration-test/testdb/amwFileDbIntegrationEmpty;AUTO_SERVER=TRUE;
