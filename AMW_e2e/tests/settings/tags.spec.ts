...
test.describe('Tags -CRUD', () => {
    test('should create, read and delete a tag', async ({ page }) => {
        ...
        await expect(page.locator('ngb-toast :has-text("Tag added.")')).toBeVisible();

        await page.locator('#tagName').fill('test-tag');
        await page.locator('#tagName').press('Enter');
        await expect(page.locator('#tagName')).toBeEmpty();
        await expect(page.locator('ngb-toast :has-text("Tag with name test-tag already exists.")')).toBeVisible();
        ...
        await expect(page.locator('ngb-toast :has-text("Tag deleted.")')).toBeVisible();
    });
});
